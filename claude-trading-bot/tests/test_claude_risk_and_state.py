...
def test_drawdown_19_99_does_not_latch(limits):
    limits.check_drawdown(Decimal("199.90"))  # 19.99% of 1000 -- must not raise


def test_drawdown_exactly_20_00_latches(limits):
    with pytest.raises(risk_engine_guard.DrawdownLimitBreached) as exc_info:
        limits.check_drawdown(Decimal("200.00"))
    assert exc_info.value.drawdown_pct == Decimal("20.00")


def test_drawdown_above_20_remains_latched(limits):
    with pytest.raises(risk_engine_guard.DrawdownLimitBreached):
        limits.check_drawdown(Decimal("250.00"))
...
def test_only_telegram_control_patch_calls_state_mutating_owner_functions():
    ...
    confirm_restart() must only ever be called from telegram_control_patch.py
    (the owner-gated handler) or claude_state.py itself (tests aside) --
    proving no other module, and therefore no AI/mailbox/internal/scheduler
    code path anywhere in this bot, can reach them."""
    mutators = ("claude_state.arm(", "claude_state.disarm(", "claude_state.stop(",
                "claude_state.issue_restart_challenge(", "claude_state.confirm_restart(")
    offenders = []
    for path in BOT_DIR.glob("*.py"):
        if path.name in {"telegram_control_patch.py", "claude_state.py"}:
            continue
        text = path.read_text(encoding="utf-8")
        for m in mutators:
            if m in text:
