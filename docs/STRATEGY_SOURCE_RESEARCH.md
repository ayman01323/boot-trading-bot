...
`learnerbot/strategy_external_research.py` builds a bounded fresh external evidence pack whenever the Strategy Lab
research report is assembled. This happens upstream of the three-agent strategy review, so GPT, Gemini and Copilot see
the same timestamped external evidence inside the sanitised loss-forensics payload before they form a strategy view.

The fresh collectors are intentionally small and auditable:
