...
def normalise_key(value: str) -> str:
    ...
    if key not in REPORTS:
        raise ValueError(
            "unknown report; use trade, engineering, strategy, factory, engineering_ai or seven_agent"
        )
    return key
...
def save_schedule(
    ...
    *,
    ...
    for key in REPORTS:
        clean[key] = max(
            MIN_INTERVAL_HOURS,
            min(MAX_INTERVAL_HOURS, int(schedule.get(key, clean[key]))),
        )
    payload = {
        "schema_version": 1,
        "updated_epoch": now,
        "changed_by": str(changed_by)[:120],
        "hours": clean,
    }
...
def set_interval(app, report: str, hours: int, *, changed_by: str) -> dict:
    ...
    if hours < MIN_INTERVAL_HOURS:
        raise ValueError(
            f"automatic report interval cannot be less than {MIN_INTERVAL_HOURS} hours"
        )
    if hours > MAX_INTERVAL_HOURS:
        raise ValueError(
            f"automatic report interval cannot exceed {MAX_INTERVAL_HOURS} hours"
        )
    ...
    if not state.get("initialised_epoch"):
        ...
        for key in REPORTS:
            runs.setdefault(
                key,
                {
                    "last_attempt_epoch": int(state.get("initialised_epoch") or now),
                    "last_success_epoch": 0,
                    "last_status": "INITIALISED",
                },
            )
...
def due_reports(app, *, now: int | None = None) -> list[str]:
    ...
    state = load_state(app, now=now)
    out = []
    # Factory is last so evidence-producing reports that become due on the same
    # controller pass are queued before the central Factory adjudication.
    order = (
        "trade",
        "engineering",
        "strategy",
    ...
    for key in order:
        ...
        base = max(
            int(row.get("last_attempt_epoch") or 0),
            int(row.get("last_success_epoch") or 0),
        )
...
def snapshot(app, *, now: int | None = None) -> dict:
    ...
    for key, meta in REPORTS.items():
        ...
        base = max(
            int(run.get("last_attempt_epoch") or 0),
            int(run.get("last_success_epoch") or 0),
        )
        ...
        rows.append(
            {
                "key": key,
                "label": meta["label"],
                "hours": int(schedule[key]),
                "mode": meta["mode"],
                "last_status": str(run.get("last_status") or "NEVER"),
                "last_attempt_epoch": int(run.get("last_attempt_epoch") or 0),
                "last_success_epoch": int(run.get("last_success_epoch") or 0),
    ...
    return {
        "schema_version": 1,
        "generated_epoch": now,
        "minimum_automatic_hours": MIN_INTERVAL_HOURS,
        "reports": rows,
    }
