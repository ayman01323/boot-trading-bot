...
def _report(app):
    ...
    platform_auto = _b(auto_platform.get('auto_trading_enabled'), False)
    ...
    for u in targets:
        ...
        try:
            ...
            evm_signing = False
        summary = [f"platform_auto={str(platform_auto).lower()}", f"evm_signing={str(evm_signing).lower()}"]
        for c in load_chains(app, enabled_only=True):
            ...
            effective = p_live and platform_auto and u_live and u_auto and mode == 'ARMED' and can_auto and evm_signing
            summary.append(f"{c.slug}={str(effective).lower()}({str(p_live).lower()}/{str(u_live).lower()}/{str(u_auto).lower()}/{mode})")
        try:
            ...
            summary.append(f"solana={str(seffective).lower()}({str(senabled).lower()}/{str(slive).lower()}/{str(ssign).lower()})")
        except Exception as exc:
            summary.append(f"solana=false(error={type(exc).__name__})")
        print("[live-status-summary] " + " ".join(summary))
...
def _app_with_probe():
    ...
    try:
        ...
        print(f"[live-status-summary] probe_error={type(exc).__name__}")
    return app
