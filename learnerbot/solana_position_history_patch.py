...
def classify_swap(result: dict, wallet: str) -> dict | None:
    """Keep transaction-visible token balances needed to prove position closure."""
...
def _match_events(wallet: str, events: list[dict]):
    ...
    A position is eligible for position-level quality metrics only when the BUY
    cycle starts from visible zero balance, every subsequent swap agrees with the
    previous visible post-balance, reconstructed quantity stays consistent, and
    the final SELL proves post-balance zero. Partial sells, carried-in inventory,
    token transfers between swaps, or missing balance proof therefore cannot be
    promoted into a fabricated closed win/loss.
    """
