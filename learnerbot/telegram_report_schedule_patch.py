...
def _schedule_text(app) -> str:
    ...
    lines.extend(
        [
            ...
            "<i>Manual runs are allowed at any time. Automatic intervals cannot be set below 4 hours.</i>",
        ]
    )
    return "\n".join(lines)
...
def _run_report(app, key: str, *, manual: bool) -> tuple[bool, str]:
    ...
    script = root / "scripts" / "monitor_factory_operations.py"
    ...
    while True:
        try:
            for key in _sched.due_reports(app):
                ...
                print(
                    f"[report-scheduler] report={key} success={str(ok).lower()} "
                    f"detail={detail[-300:]!r}"
                )
        except Exception as exc:
            print(f"[report-scheduler] {type(exc).__name__}: {exc}")
        # Cheap due-time check only. It makes no AI/model call when no report is due.
        time.sleep(300)
...
def _start(app):
    ...
    with _LOCK:
        ...
        thread = threading.Thread(
            ...
            args=(app,),
            name="report-schedule-controller",
            daemon=True,
        )
        thread.start()
        _STARTED = True
...
def set_commands(token: str):
    ...
    try:
        ...
        for row in all_users(csv_dir):
            ...
            current = _tg._json(
                "getMyCommands", token, payload={"scope": scope}, timeout=15
            ) or []
            _tg._json(
                "setMyCommands",
                token,
                payload={"commands": _command_set(current), "scope": scope},
                timeout=15,
            )
    except Exception as exc:
    ...
    if tid is not None and text.startswith("/"):
        ...
        if cmd in {"/aireports", "/aifrequency", "/airun"}:
            ...
            if cmd == "/aifrequency":
                ...
                if len(bits) != 2:
                    _ui._send(
                        app,
                        tid,
                        "Usage: <code>/aifrequency report hours</code>\n"
                        "Example: <code>/aifrequency factory 6</code>",
                    )
                    return
                try:
                    result = _sched.set_interval(
                        app,
                        bits[0],
                        int(bits[1]),
                        changed_by=f"telegram-master:{tid}",
                    )
                    _ui._send(
                        app,
                        tid,
                        f"✅ <b>{_safe(_sched.REPORTS[result['key']]['label'])}</b> "
                        f"set to every <b>{result['hours']}h</b>.\n\n"
                        + _schedule_text(app),
                    )
                except Exception as exc:
            ...
            _ui._send(
                app,
                tid,
                f"▶️ Running <b>{_safe(_sched.REPORTS[key]['label'])}</b> now. "
                "Automatic frequency is unchanged.",
            )
            ...
            def worker():
                ...
                _ui._send(
                    ...
                    tid,
                    ("✅" if ok else "⚠️")
                    + f" <b>{_safe(_sched.REPORTS[key]['label'])}</b> "
