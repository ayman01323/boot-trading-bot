...
def _snapshot(app, tid):
    ...
    result["evm_history_ready"] = bool(providers) and all(value != "MISSING" for value in providers.values())
    # Backward compatibility for the older report renderer. This field now means
    # a usable EVM history provider exists for every enabled EVM chain, not that
    # an Etherscan key must exist.
    result["etherscan_configured"] = result["evm_history_ready"]
    return result


def _provider_summary(providers: dict) -> str:
    if not providers:
        return "NONE"
    unique = sorted(set(str(value).upper() for value in providers.values()))
    return "+".join(unique)
...
def build_report(app, tid) -> str:
    ...
    if snapshot.get("evm_history_ready"):
        replacement = f"🟢 EVM history provider: <b>{summary}</b>"
    else:
        missing = ", ".join(slug.upper() for slug, provider in providers.items() if provider == "MISSING") or "enabled EVM chains"
        replacement = f"🔴 EVM history provider missing: <b>{missing}</b>"
...
def _publish_startup_health(app):
    ...
    ready = bool(providers) and all(provider != "MISSING" for provider in providers.values())
    safe = {
    ...
    print(
        ...
        % (_provider_summary(providers), safe["polygon_focus"])
    )
            ...
            for master_tid in masters:
                try:
                    _health.send_message(
                        ...
                        master_tid,
                        "🔴 <b>EVM SiBot history provider missing</b>\n"
                        f"No usable history provider is configured for <b>{missing}</b>. "
                        "Use <code>/whynotrade</code> for the funnel.",
                        parse_mode="HTML",
...
def install():
    if getattr(_health, "_history_provider_health_patch_installed", False):
        return
    _health._snapshot = _snapshot
    _health.build_report = build_report
    _health._publish_startup_health = _publish_startup_health
    _health._history_provider_health_patch_installed = True
