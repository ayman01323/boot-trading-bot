...
PUBLIC_CLAUDE_TARGETS = ("claude-general", "claude-coding")
...
def parse_chat_target(value: str) -> tuple[str, str]:
    """Return (canonical broker agent, Claude division).

    Bare ``claude`` is deliberately ambiguous and therefore rejected for
    operator-facing routing. Other agents pass through unchanged.
    """
...
def general_message(body: str) -> str:
    ...
    if "CLAUDE_DIVISION: GENERAL" in text.upper():
        return text
    return (
        "CLAUDE_DIVISION: GENERAL\n"
        "CLAUDE_IDENTITY: AUTOMATED_GENERAL\n"
        "ROUTING_RULE: governance/research/advisory only; no repository mutation authority\n\n"
        + text
    )
...
def repository_name() -> str:
    return str(os.environ.get("GITHUB_REPOSITORY") or "").strip() or _repo_from_git()
...
def build_coding_request(
    ...
    *,
    ...
    thread_id = str(thread_id or "").strip()[:120]
    payload = (
        "GPT_TO_CLAUDE\n"
        ...
        + (f"subject: {subject}\n" if subject else "")
        + "constraints: persistent repository/coding division; branch/handoff rules apply; no silent deploy/trade/LIVE/risk/capital/wallet/signing changes; no secrets\n\n"
        + text[:7000]
        + "\n"
    )
...
def coding_reply_identity(text: str) -> tuple[str, str]:
    _, headers = _headers(text)
    return str(headers.get("division") or "").upper(), str(headers.get("identity") or "").upper()


def validate_coding_reply(text: str, *, expected_message_id: str) -> dict[str, str]:
    """Validate the achievable provenance floor for the Git mailbox.

    This is not cryptographic session attestation. It *does* fail closed when a
    consumer tries to treat a reply as Claude Coding without the required
    division/identity/correlation headers.
    """
    expected = str(expected_message_id or "").strip()
    if not _MESSAGE_ID_RE.fullmatch(expected):
        raise ValueError("invalid expected Claude Coding message id")
    first, headers = _headers(text)
    if first != "CLAUDE_TO_GPT":
        raise ValueError("UNVERIFIED_CLAUDE_CODING_REPLY: invalid mailbox prefix")
    in_reply_to = str(headers.get("in_reply_to") or "").strip()
    division = str(headers.get("division") or "").strip().upper()
    identity = str(headers.get("identity") or "").strip().upper()
    if in_reply_to != expected:
        raise ValueError("UNVERIFIED_CLAUDE_CODING_REPLY: correlation mismatch")
    if division != "CODING":
        raise ValueError("UNVERIFIED_CLAUDE_CODING_REPLY: division is not CODING")
    if identity != "PERSISTENT_AGENT":
        raise ValueError("UNVERIFIED_CLAUDE_CODING_REPLY: persistent identity not attested")
    status = str(headers.get("status") or "RESPONSE").strip().upper()
    return {
        "in_reply_to": in_reply_to,
        "division": division,
        "identity": identity,
        "status": status,
        "verification": "HEADER_VERIFIED_NOT_CRYPTOGRAPHIC",
    }


def fetch_verified_coding_reply(
    *,
    ...
    if not repo or not token:
        raise RuntimeError("GitHub repository/authentication unavailable for Claude Coding reply verification")
    text, _ = fetch_fixed_file(repo, CLAUDE_TO_GPT_PATH, token=token)
    verified = validate_coding_reply(text, expected_message_id=expected_message_id)
