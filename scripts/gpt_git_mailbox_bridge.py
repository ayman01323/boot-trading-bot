...
def normalize_gpt_request(text: str) -> tuple[str, str]:
    ...
    envelope = (
        ...
        "The response must be labelled accordingly. Treat the request as communication/analysis only.\n\n"
        f"{payload}\n"
    )
...
def select_pending(repo: str, *, token: str) -> tuple[bool, str, str]:
    ...
    try:
        outgoing, _ = fetch_fixed_file(repo, CLAUDE_API_TO_GPT_PATH, token=token)
...
def _reply_message_id(request_id: str) -> str:
    ...
    return f"claude-api-reply-{digest}"
...
def build_claude_mailbox_reply(request_id: str, bus_reply: str) -> str:
    ...
    return (
        ...
        "[IDENTITY NOTICE] This response was generated by the stateless Claude/Anthropic API bridge. "
        "It is not a message authored by the persistent/interactive Claude agent.\n\n"
        f"{text}\n"
    )
...
def publish_reply(repo: str, *, token: str, request_id: str, bus_reply: str) -> None:
    ...
    existing_sha = ""
    try:
        _, existing_sha = fetch_fixed_file(repo, CLAUDE_API_TO_GPT_PATH, token=token)
    except RuntimeError as exc:
        if "HTTP 404" not in str(exc):
            raise
    payload: dict[str, Any] = {
        "message": f"Claude API responder reply to GPT mailbox {request_id}",
