...
class MandatoryShadowPoolCheck:
    ...
    def _solana(self, mint: str, base_evidence: dict[str, Any]) -> PoolCheckDecision:
        ...
        reason = str((result or {}).get("reason") or (result or {}).get("reason_code") or "poolcheck")
        merged = {**base_evidence, **dict((result or {}).get("evidence") or {})}
        if verdict in {"HARD_BLOCK", "COOLING", "SHADOW_ONLY"}:
            return self._decision(verdict, reason, evidence=merged)
