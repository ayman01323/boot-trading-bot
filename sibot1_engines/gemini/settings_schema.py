...
class Settings:
    ...
    min_volume_usd: Decimal = Decimal("200")
    min_volume_liquidity_ratio: Decimal = Decimal("0.02")
    max_volume_liquidity_ratio: Decimal = Decimal("10")
    min_liquidity_velocity_pct: Decimal = Decimal("-35")
    min_lp_locked_pct_prefilter: Decimal = Decimal("25")
    signal_cooldown_ms: int = 5 * 60 * 1000
...
def load_settings(path: str | Path) -> Settings:
    ...
    return Settings(
        ...
        min_liquidity_usd=Decimal(str(row.get("min_liquidity_usd") or "5000")),
        min_volume_usd=Decimal(str(row.get("min_volume_usd") or "200")),
        min_volume_liquidity_ratio=Decimal(str(row.get("min_volume_liquidity_ratio") or "0.02")),
        max_volume_liquidity_ratio=Decimal(str(row.get("max_volume_liquidity_ratio") or "10")),
        min_liquidity_velocity_pct=Decimal(str(row.get("min_liquidity_velocity_pct") or "-35")),
        min_lp_locked_pct_prefilter=Decimal(str(row.get("min_lp_locked_pct_prefilter") or "25")),
        signal_cooldown_ms=int(row.get("signal_cooldown_ms") or 5 * 60 * 1000),
