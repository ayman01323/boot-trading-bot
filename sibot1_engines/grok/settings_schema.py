...
class Settings:
    ...
    min_volume_velocity: Decimal = Decimal("1.0")
...
def load_settings(path: str | Path) -> Settings:
    ...
    return Settings(
        ...
        trade_amount=Decimal(str(row.get("trade_amount") or "1")), min_confidence=Decimal(str(row.get("min_confidence") or "0.60")),
        min_volume_velocity=Decimal(str(row.get("min_volume_velocity") or "1.0")), take_profit_pct=Decimal(str(row.get("take_profit_pct") or "0.035")),
