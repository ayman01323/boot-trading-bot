...
def _get_json(url: str, timeout: float) -> Any:
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "grok-known-assets-paper/0.2",
        },
    )
...
def _select_solana_pair(payload: Any) -> dict[str, Any]:
    ...
    for pair in payload:
        ...
        if "USDC" not in {quote_symbol, base_symbol} and "USDT" not in {quote_symbol, base_symbol}:
            continue
        try:
...
class SolanaNativeLiveFeed:
    """Real public-market collector for native SOL PAPER trading only.

    It never receives a wallet/private key and cannot sign or broadcast. Non-native
    Solana assets and EVM assets remain fail-closed until dedicated route/pool-safety
    collectors exist for them.
    """
    ...
    def _append_history(self, asset_key: str, ts: float, price: float) -> list[tuple[float, float]]:
        ...
        self.journal.set_state(
            f"paper_feed_history:{asset_key}",
            [[round(t, 3), p] for t, p in rows[-500:]],
        )
        return rows
    ...
    def _metrics(self, rows: list[tuple[float, float]], now: float, current: float) -> tuple[float, float, float, float]:
        ...
        def ret(old: float) -> float:
            return (current / old - 1.0) * 100.0

        five_minute = [(t, p) for t, p in rows if t >= now - 300]
        one_step_returns: list[float] = []
        for (_, a), (_, b) in zip(five_minute, five_minute[1:]):
            if a > 0 and b > 0:
                one_step_returns.append((b / a - 1.0) * 100.0)
        vol_5m = statistics.pstdev(one_step_returns) if len(one_step_returns) >= 2 else 0.0
    ...
    def collect(self, asset: Asset, *, now: float | None = None) -> ValidatedSnapshotEnvelope:
        ...
        if ask < bid:
            ...
            ask = max(ask, mid)
        if ask < bid:
            raise FeedSafetyError("JUPITER_CROSSED_SYNTHETIC_MARKET")
        ...
        observation = ProviderObservation(
            ...
            ret_15m_pct=ret_15m,
            vol_5m_pct=vol_5m,
            pool_id=str(pair.get("pairAddress") or ""),
        )
        route = JupiterRouteEvidence(
            ...
            ask=ask,
            reverse_bid=bid,
            impact_bps=_price_impact_bps(sell, buy),
            fees_bps=self.settings.assumed_fee_bps,
            slippage_bps=float(self.settings.slippage_bps),
            route_id="|".join(_route_pool_ids(sell, buy)),
            asset_pool_ids=_route_pool_ids(sell, buy),
        )
