...
def test_readiness_display_values_derive_from_integer_canary_cap():
    assert ENTRY_TARGET_SOL == lc.TARGET_LAMPORTS / 1_000_000_000
    assert HARD_MAX_ENTRY_SOL == lc.HARD_CAP_LAMPORTS / 1_000_000_000


def test_exit_wallet_preflight_requires_position_plus_fee_reserve():
    need = 8_800_000
    ok, _ = lx.preflight_exit_funding(
        approved_exit_input_lamports=need,
        executor=_BalanceExecutor(need + lc.SOL_FEE_RESERVE_LAMPORTS),
    )
    assert ok is True

    ok, reason = lx.preflight_exit_funding(
        approved_exit_input_lamports=need,
        executor=_BalanceExecutor(need + lc.SOL_FEE_RESERVE_LAMPORTS - 1),
    )
    assert ok is False
    assert "insufficient spendable SOL" in reason
...
def test_exit_rejected_before_swap_when_live_balance_is_insufficient(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **k: (False, "insufficient spendable SOL for approved exit"),
    )
    called = []
    ...
    assert "insufficient spendable SOL" in str(row["outcome_detail"])
