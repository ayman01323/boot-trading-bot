...
def test_solana_profit_factor_recent_and_drawdown_gates_in_isolated_process():
    _run(r'''
        ...
        assert cfg["leader_recent_activity_hours"] == "6"
        good={
...
def test_solana_qualified_leaders_are_prioritised_by_recent_activity_bucket():
    source = (ROOT / "learnerbot" / "solana_profit_guard_patch.py").read_text(encoding="utf-8")
    assert 'leader_recent_activity_hours' in source
    assert 'last_activity_ts' in source
    assert 'activity_cutoff' in source
    # Recency is a preference after qualification, not a replacement for the
    # existing profitability gate.
    assert 'if _historical_ok(m, cfg)' in source
    assert '1 if int(x[1].get("last_activity_ts") or 0) >= activity_cutoff else 0' in source
...
def test_solana_quality_page_shows_profit_guards_in_isolated_process():
    _run(r'''
        ...
        ui._sol.settings=lambda app:{
            ...
            "leader_recent_activity_hours":"6",
        ...
        assert "active within <b>6h" in text
