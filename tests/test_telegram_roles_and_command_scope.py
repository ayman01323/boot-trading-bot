...
def test_blue_command_menu_is_role_scoped(monkeypatch, tmp_path):
    ...
    assert "control" not in chat_scopes["5882384847"]
    assert "engine" not in chat_scopes["461513364"]
    assert "wallet" in chat_scopes["5882384847"]
    assert "sibot" in chat_scopes["461513364"]
